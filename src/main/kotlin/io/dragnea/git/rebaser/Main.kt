...
fun Git.needsRebasing(pr: GHPullRequest): Boolean {
    val baseRef = pr.base.ref
    val headRef = pr.head.ref
    ...
    return log().addRange(headCommit, baseCommit).call().toList().isNotEmpty()
}

fun Git.computeAllPrsToRebase(allPrs: List<GHPullRequest>, base: String = "master"): List<GHPullRequest> {
    val prsToRebase = allPrs.filter { it.base.ref == base && needsRebasing(it) && isSafePr(it) }
...
fun main() {
    ...
    } while (allPrsToRebase.isNotEmpty())
}
